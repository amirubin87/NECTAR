...
Parameters:
	...
	Additional input parameters for NECTAR_W: 
...
Usage examples:
	java -jar NECTAR_Q ./net.txt ./
	java -jar NECTAR_W ./net.txt ./
		These  are examples with the minimum parameters needed.
	java -jar NECTAR_Q ./net.txt ./ 1.1,1.2,1.3,1.4,1.5,2.0,2.5,3.0,3.5,4.0 0.8 3 30
		NECTAR runs, maximizing Overlapping modularity, with 10 different beta values. After the third iteration communities which share 70% of their nodes will be merged.
		The algorithm will stop after 30 iterations.
	java -jar NECTAR_W ./net.txt ./ 1.5,3.5,5.0,10.0 0.7 4 30 3 99
		NECTAR runs, maximizing WOCC, with 4 different beta values. The first partitioning is all 3-sized cliques. After the forth iteration communities which share 70% of their nodes will be merged.
